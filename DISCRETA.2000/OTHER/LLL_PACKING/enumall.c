...
int onlinesolutiontest(DOUBLE *w, int z, long c1, long nom, long denom, int selection, int *permutation, int KMz);
...
#if VERBOSE > 0              
		if (loops%1000000==0) {
			printf("%ld loops, best packing: %ld, Number of solutions: %ld\n",loops,bestpacking,nosolutions);
			fflush(stdout);
		}
#endif
...
/*printf("*****************************\n");*/
				if (exacttest(w[j],z,Fq)==1) {          
					...
					onlinesolutiontest(w[t],z,solutionnorm,nom,denom,selection,permutation,KMz); 
...
/* Here comes some additional output. */
	...
	printf("Best packing = %ld out of %ld blocks\n",bestpacking,maximal_number_of_blocks);
...
int pruneprimal(DOUBLE *w, int t, int z, DOUBLE Fq, int *firstlist, int *firstp) {
	...
	for (i=0;i<firstlist[firstp[t+1]];i++) {
		...
		if (f>0) {
			...
			if (fabs(w[0]) > Fq*(1.0+EPSILON)) return 1;
/*			if (fabs(w[0]) > EPSILON) return 1; 			*/
		}
	} 
	return 0;
}
...
int onlinesolutiontest(DOUBLE *w, int z, long c1, long nom, long denom, int selection, int *permutation, int KMz) {
	...
	*/
	...
	/* \|labs(b[j][z-1])==1.0| means the solution is inhomogenous.
	   ...
	   a solution.
	*/   
	if (( fabs(fabs(w[z-1])-(DOUBLE)nom)>EPSILON) || (fabs(w[z-2])<EPSILON)) return 0; 
	/* The next test assures that we only get solutions for the $\lambda$'s
   	which we want to test.
	*/ 
	if (fabs(fabs(w[z-2])-(DOUBLE)nom)>EPSILON) return 0; 
	F = 1;
	...
	if (F==1) {
		...
		if ((long)round(w[z-1])>0) {
				noblocks = midpoint-(long)round(w[0]);
			} else {
				noblocks = midpoint+(long)round(w[0]);
			}

		if (noblocks>bestpacking) {
			bestpacking = noblocks;
		}
...
/*
			for (i=0;i<upper+2;i++) {
				printf("%d ",(int)round(w[i]));
			}  
			printf("\n");
*/			
			for (i=KMz+1;i<upper;i++) {
			...
			for (i=0;i<s_sel-3;i++) {
				...
				for (l=i0;l<=i;l++) {
					if (permutation[l]==i) {
						u=labs((long)round(w[i0])-(long)round(w[z-1])*nom)/denom;
						i0++;
						break;
					}
				}
