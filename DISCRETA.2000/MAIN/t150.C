...
INT Transpose(BYTE *geo_file1, BYTE *geo_file2, 
	INT f_range, INT range_f, INT range_l,
	INT f_v, INT f_vv);
INT transpose(MATRIX_OP I, 
...
void usage()
{
	printf("usage: t150.out [options] geo_file\n");
	printf("transposes incidence matrices\n");
...
int main(int argc, char **argv)
{
	...
	if (f_input_geo) {
		strcat(geo_file2, "_t.geo");
		Transpose(geo_file1, geo_file2, f_range, range_f, range_l, f_v, f_vv);
		}
	else  {
	...
	INT f_v, INT f_vv)
{
	...
	printf("transposing %s, result written to %s\n", geo_file1, geo_file2); fflush(stdout);
	...
	INT f_v, INT f_vv)
{
	...
	if (f_aut_gen)
		vec_induce_action_on_columns(aut_gen, I);
	I->transpose(&It);
	It.swap(I);
	a = *f_ddp;
	*f_ddp = *f_ddb;
