...
void possolve(int **lo, int **hi, equation *eqn, int *lorhs, int *hirhs, int *neqn, int numeqn, int numvar)
{
   register int i,j;
   register term *e;
	boolean hopeless;
...
/* First step: 
   ...
   solely $\in\{0,1\}$.
*/
	active = (boolean**)calloc(numvar,sizeof(boolean*));
	for (i=0;i<numvar;i++) active[i] = (boolean*)calloc(numeqn,sizeof(boolean));
...
/* Finally the recursion starts. */
	if (!hopeless) solve(0,lo,hi,numvar,lorhs,hirhs,eqn,neqn,numeqn);
	printf("Loops: %ld\n",loops);
	
	for (i=0;i<numvar;i++) free(active[i]);
	free(active);
...
/*  First the lower and upper bounds on the variable are written. */
        ...
        for (i = 0; i < numvar; ++i) {
            ...
            if (lo[level][i] > 0) fprintf(out_txt,"x%d >= %d, ",i,lo[level][i]);
            fprintf(out_txt,"x%d <= %d",i,hi[level][i]);
        }
...
*/
void solve(int level, int **lo, int **hi, int numvar,int *lorhs, int *hirhs, equation *eqn,int *neqn,int numeqn) {
	register int i,j,losum,hisum,eic,eiv,lx,hx;
	int ok;
	int nfree;
	int xlo,xhi,isplit,mindiff;
#if 1<0
	int mathon_row, mathon_number, mathon_column, mathon_i;
#endif	
	term *e;

/*printf("Solve: %d\n",level);*/
	if ((stoploops>0)&&(loops>=stoploops)) {
		exit(0);
	}
	loops++;	
	if (level>0) {
		for (i = 0; i < numvar; i++) {
			 lo[level][i] = lo[level-1][i];
			 hi[level][i] = hi[level-1][i];
		}
		for (i = 0; i < numeqn; ++i) active[level][i] = active[level-1][i];
	}
...
*/   
			if (unitcoeffs[j]) {
				...
				hisum = 0;
				for (i = neqn[j]; --i >= 0;) {
					losum += lo[level][e[i].var];   /* lowest possible rhs */
					hisum += hi[level][e[i].var];   /* highest possible rhs */
				}
				if (losum > hirhs[j] || hisum < lorhs[j]) return;
...
*/
				...
				for (i = neqn[j]; --i >= 0;) {
					...
					* equations again. */
						...
						if (lo[level][eiv] != hi[level][eiv]) nfree++;
					}
				}
			} /* if (unitcoeffs[j]) */
...
*/   
				...
				hisum = 0;
				for (i = neqn[j]; --i >= 0;) {
					losum += e[i].coeff * lo[level][e[i].var];
					hisum += e[i].coeff * hi[level][e[i].var];
				}
				if (losum > hirhs[j] || hisum < lorhs[j]) return;
...
*/
				nfree = 0;
				for (i = neqn[j]; --i >= 0;) {
					if (hi[level][e[i].var] != lo[level][e[i].var]) {
						eic = e[i].coeff;
						eiv = e[i].var;
						hx = eic * hi[level][eiv];
						lx = eic * lo[level][eiv];
						...
						xhi = hirhs[j] + lx - losum;
						if (xlo > lx) {
							lo[level][eiv] = (xlo + eic - 1) / eic;
							if (lo[level][eiv] > hi[level][eiv]) return;
							ok = 0;
						}
						if (xhi < hx) {
							hi[level][eiv] = xhi / eic;
							if (lo[level][eiv] > hi[level][eiv]) return;
							ok = 0;
						}
						if (lo[level][eiv] != hi[level][eiv]) nfree++;
...
*/         
#if 1<0
			if ((nfree<mathon_number)&&(nfree>0)) {
				mathon_row = j;
				mathon_number = nfree;
			}
#endif			
			if (ok > 0 && nfree == 0) active[level][j] = FALSE;
		}
	}
...
*/
	...
	isplit = -1;
	for (i = 0; i < numvar; i++) {
		if (hi[level][i] != lo[level][i] && hi[level][i] - lo[level][i] < mindiff) {
			isplit = i;
			mindiff = hi[level][i] - lo[level][i];
			break;   /* Only 0,1 - variables!! */
		}
	}

#if 1<0
	isplit = mathon_column;	
	printf("mathon_column: %d ---",mathon_column);
#endif
...
/* If \|isplit| $< 0$ we have found a solution. Otherwise we try to
  ...
  by \|pruneqn|.
*/
	if (isplit < 0)  (solproc)(lo[level],numvar);
	else {
		if (level == 0) {
			varprune(lo[level],hi[level],lorhs,hirhs,eqn,neqn,numeqn);
...
*/
		for (i = 0; i <= mindiff; ++i) {
			hi[level][isplit] = lo[level][isplit];
			solve(level+1,lo,hi,numvar,lorhs,hirhs,eqn,neqn,numeqn);
			++lo[level][isplit];
		}
	}
}
